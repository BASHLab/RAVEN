...
def find_all_linear_names(model):
    ...
    multimodal_keywords = ['mm_projector', 'vision_tower', 'vision_resampler', "audio_tower", "mm_allingment_module"]
    ...
    # print("before", list(lora_module_names))
    # print("\n\n\n")
    if 'lm_head' in lora_module_names: # needed for 16-bit
        lora_module_names.remove('lm_head')
    # print("after", list(lora_module_names))
...
def safe_save_model_for_hf_trainer(trainer: Trainer,
    ...
    if getattr(trainer.args, "tune_mm_mlp_adapter", False):
    ...
    elif getattr(trainer.args, "tune_mm_mlp_adapter_s", False):
        # Only save Adapter
        keys_to_match = ['mm_projector_s']
        if getattr(trainer.args, "use_im_start_end", False):
            keys_to_match.extend(['embed_tokens', 'embed_in'])
        ...
        if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
            if current_folder.startswith('checkpoint-'):
                mm_projector_folder = os.path.join(parent_folder, "mm_projector_s")
                os.makedirs(mm_projector_folder, exist_ok=True)
                torch.save(weight_to_save, os.path.join(mm_projector_folder, f'{current_folder}.bin'))
            else:
                torch.save(weight_to_save, os.path.join(output_dir, f'mm_projector_s.bin'))
    ...
    elif getattr(trainer.args, "pretrain_mm_mlp_adapter_s", False):
        # Only save Adapter
        print("here")
        keys_to_match = ['mm_projector_s']
        if getattr(trainer.args, "use_im_start_end", False):
...
class RAVENTrainer(Trainer):
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            if self.args.mm_projector_lr is not None:
                ...
                mm_alingment_module_parameters = [name for name, _ in opt_model.named_parameters() if "mm_allingment_module" in name]
                optimizer_grouped_parameters = [
                    {
                        "params": [
                            p for n, p in opt_model.named_parameters() if (n in decay_parameters and p.requires_grad and n not in mm_alingment_module_parameters)
                        ],
                        "weight_decay": self.args.weight_decay,
                    },
                    {
                        "params": [
                            p for n, p in opt_model.named_parameters() if (n not in decay_parameters and p.requires_grad and n not in mm_alingment_module_parameters)
                        ],
                        "weight_decay": 0.0,
                    },
                    {
                        "params": [
                            p for n, p in opt_model.named_parameters() if (n in decay_parameters and n in mm_alingment_module_parameters and p.requires_grad)
                        ],
                        "weight_decay": self.args.weight_decay,
                        "lr": self.args.mm_alingment_module_lr,
                    },
                    {
                        "params": [
                            p for n, p in opt_model.named_parameters() if (n not in decay_parameters and n in mm_alingment_module_parameters and p.requires_grad)
                        ],
                        "weight_decay": 0.0,
                        "lr": self.args.mm_alingment_module_lr,
                    },
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        if getattr(self.args, 'tune_mm_mlp_adapter', False):
            ...
            if self.args.lora_enable:
                ...
                super(RAVENTrainer, self)._save_checkpoint(model, trial, metrics)
            else:
                super(RAVENTrainer, self)._save_checkpoint(model, trial, metrics)
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        if getattr(self.args, 'tune_mm_mlp_adapter', False):
            ...
            super(RAVENTrainer, self)._save(output_dir, state_dict)
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        """
        # print(type(inputs), input.keys())
        if self.label_smoother is not None and "labels" in inputs:
            labels, am_attn = inputs.pop("labels")
