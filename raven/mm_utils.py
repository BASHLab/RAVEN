...
def process_sensor_file(sensor_path):
...
def process_video(video_path, processor, s=None, e=None, aspect_ratio='pad', num_frames=NUM_FRAMES, va=False, sensor_path=None):
    ...
    if va:
        ...
        if sensor_path is not None:
            sensor = process_sensor_file(sensor_path)
            sensor = torch.from_numpy(sensor)
        else:
            sensor = torch.zeros(120, 6)
        video = video = {'video': video, 'audio': audio, 'sensor': sensor}
