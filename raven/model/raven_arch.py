...
from .allignment import build_mm_allingment_module
from ..constants import IGNORE_INDEX, NUM_FRAMES, MODAL_INDEX_MAP


class RAVENMetaModel:

    def __init__(self, config):
        super(RAVENMetaModel, self).__init__(config)
        ...
        self.mm_allingment_module = build_mm_allingment_module()
        for p in self.mm_allingment_module.parameters():
    ...
    def initialize_audio_modules(self, model_args, fsdp=None):
        ...
        self.config.mm_hidden_size_a = self.audio_tower_cfg.encoder_embed_dim
        self.config.hidden_size_a = self.audio_tower_cfg.hidden_size
...
class RAVENMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        new_input_embeds = self.get_model().mm_allingment_module(new_input_embeds)
