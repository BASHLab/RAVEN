...
from .raven_qwen2 import RAVENQwen2ForCausalLM, RAVENQwen2Config


VLLMs = {
    "raven_qwen2": RAVENQwen2ForCausalLM,
}

VLLMConfigs = {
    "raven_qwen2": RAVENQwen2Config
}
...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", device="cuda", use_flash_attn=False, **kwargs):
    ...
    if 'lora' in model_name.lower() or 'qlora' in model_name.lower():
        ...
        print('Loading RAVEN from base model...')
        ...
        print('Loading additional RAVEN weights...')
        ...
        model = RAVENQwen2ForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=config, **kwargs)
        mm_projector_weights = load_mm_projector(model_path, token=token)
        model.load_state_dict(mm_projector_weights, strict=False)
        
    elif 'raven' in model_type:
        # NOTE: SFT model loading
        tokenizer = AutoTokenizer.from_pretrained(model_path, use_fast=False, token=token)
        model = RAVENQwen2ForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, config=config, **kwargs)
    else:
