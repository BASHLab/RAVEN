...
def build_sensor_projector(config):
    projector_type = getattr(config, 'mm_projector_s_type', 'linear')
    mlp_gelu_match = re.match(r'^mlp(\d+)x_gelu$', projector_type)
    if mlp_gelu_match:
        mlp_depth = int(mlp_gelu_match.group(1))
        modules = [nn.Linear(config.mm_hidden_size_s, config.hidden_size_s)]
        for _ in range(1, mlp_depth):
            modules.append(nn.GELU())
            modules.append(nn.Linear(config.hidden_size_s, config.hidden_size_s))
        return nn.Sequential(*modules)
    if projector_type == "linear":
        return nn.Linear(config.mm_hidden_size_s, config.hidden_size_a)
...
### ------------------------------------ Our Allingment module starts from here ------------------------------ ####
