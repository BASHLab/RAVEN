...
from .limubert.LIMUBERT import LIMUBertModel4Pretrain, LIMUBERTConfig
...
def build_sensor_tower(sensor_tower_cfg):
    sensor_tower = getattr(sensor_tower_cfg, 'mm_sensor_tower', getattr(sensor_tower_cfg, 'sensor_tower', None))
    sensor_checkpoint = torch.load(sensor_tower, map_location="cpu")
    sensor_cfg = LIMUBERTConfig()
    model = LIMUBertModel4Pretrain(sensor_cfg)
    model.load_state_dict(sensor_checkpoint, strict=True)
    model = nn.Sequential(list(model.children())[0])
    return model, sensor_cfg
